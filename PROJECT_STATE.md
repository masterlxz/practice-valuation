...
- Usuário trouxe outra ideia pra debater depois (não implementada, só registrada): chat de IA integrado ao app, chave própria do usuário (Gemini/Claude/ChatGPT), com acesso ao banco (valuations salvas) + contexto de como o sistema funciona, sessão de conversa que reseta ao fechar (não a cada 24h). Sem arquitetura decidida ainda — fica pra próxima conversa
...
2. Discutir a ideia do chat de IA integrado (ver acima) — usuário pediu pra debater depois do commit desta sessão. Sem plano ainda, começar do zero: BYO key (Gemini/Claude/ChatGPT), acesso ao banco, contexto do sistema, sessão que reseta ao fechar o chat
