...
> Última atualização: 2026-07-10 (Sessão 5, fim — Fase 2 avançada: fundamentos via bolsai (LPA, VPA, ROE) rodando ponta a ponta pra Graham/Bancos; dividendo médio do Bazin descoberto como exclusivo do plano Pro da bolsai, adiado por decisão do usuário; primeiro indicador cripto automatizado (TVL Trend via DefiLlama, sem cadastro). Retomar por "Pendências pra próxima sessão", item 1)
...
Fase 2 — Coleta de Dados (ações BR + cripto)      [~] Em andamento (cotação via brapi e fundamentos LPA/VPA/ROE via bolsai funcionando ponta a ponta; dividendo médio do Bazin bloqueado no plano free da bolsai; TVL Trend via DefiLlama automatizado (1 dos 9 indicadores cripto) — ver Log de Sessões; CVM e os outros 8 indicadores cripto ainda faltam)
...
- [~] 2.3 — Implementar clientes de fonte de dados de cripto — **`cripto_defillama.py` (TVL Trend) concluído na Sessão 5**, único indicador com fonte 100% sem cadastro; `cripto_coingecko.py` (dado de referência, não alimenta nenhum dos 9 indicadores), `cripto_ultrasound.py` (API pública não encontrada, precisa investigar), `cripto_etherscan.py` e `cripto_stakingrewards.py` (exigem cadastro/chave) ainda faltam. `mvrv_z_score`, `puell_multiple` e `exchange_netflow` seguem sem fonte gratuita conhecida — continuam manuais (ver mapeamento de fontes)
...
1. Fase 2, continuação: escolher a próxima frente — **CVM** (fundamentos pro DCF/RNAV/Bancos, mais complexa: zip anual + parsing de CSV), **`net_issuance`/`staking_yield`** (ultrasound.money e stakingrewards.com — nenhum dos dois investigado a fundo ainda), ou revisitar o **dividendo médio do Bazin** (investigar brapi com token grátis, ou decidir assinar bolsai Pro — ver Log de Sessões, Sessão 5). `active_addresses_trend` via Etherscan já foi investigado e descartado (ver acima) — não vale reabrir sem uma ideia nova
