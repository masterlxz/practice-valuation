...
1. Replicar o mesmo padrão (`*_inputs` table + migration + entity + `domain/` + `commands/` + form em `src/models/`) pros 2 modelos restantes (RNAV, Preço Teto Projetivo) + `cripto_indicadores` (Fase 3.2/3.3) — a navegação (seletor) e a extração de tipos/componente compartilhado já estão prontas pra receber os próximos sem retrabalho
