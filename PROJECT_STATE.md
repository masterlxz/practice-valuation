...
1. Fase 5: próxima etapa é 5.3 (notificação nativa do SO — nem `@tauri-apps/plugin-notification` nem toast lib instalados ainda) pra fechar a fase. O `is_triggered`/`last_message` já exposto por `list_alert_rules` dá a base de dado que a 5.3 vai precisar
