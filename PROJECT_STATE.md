...
> Última atualização: 2026-07-12 (Sessão 12, fim — **Fase 5 completa**: 5.3 (notificação nativa do SO) finalmente validada de verdade. A notificação disparava sem erro mas não aparecia visualmente; investigação por eliminação (testes manuais via `gdbus`/`notify-send` direto no D-Bus do host) achou a causa: KDE Plasma 6/Wayland só mostra o popup de notificações com urgência **crítica** — urgência normal (única que o `tauri-plugin-notification` v2.3.3 envia no Linux, sem opção de configurar) cai só no histórico, sem popup. Fix: `tauri-plugin-notification` substituído por chamada direta ao `notify-rust` (já uma dependência transitiva) com `.urgency(Urgency::Critical)`, removendo também o `.setup()` que só existia pra obter o `AppHandle` que a notificação usava. `cargo test --lib` (42 testes) limpo, smoke test real confirmou o popup aparecendo.)
...
1. Fase 7 (chat de IA): começar pela etapa 7.1 (storage seguro da chave via `keyring-rs`, testado de verdade contra o container de dev antes de qualquer UI — é o maior risco técnico, ver Fase 7 em "Fases Detalhadas" pro desenho completo em 7 etapas)
