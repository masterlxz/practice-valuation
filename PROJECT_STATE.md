...
1. Falta só **1 modelo** pra fechar a Fase 3.2 por completo: Preço Teto Projetivo (Bazin com N anos de projeção + desconto a valor presente) — mesmo padrão (`*_inputs` table + migration + entity + `domain/` + `commands/` + form em `src/models/`)
2. Depois do 7º modelo: `cripto_indicadores` (Fase 3.3 — score de 9 indicadores verde/vermelho, tabela de série temporal, thresholds configuráveis)
