...
> Última atualização: 2026-07-10 (Sessão 5, fim — Fase 2 avançada: fundamentos via bolsai (LPA, VPA, ROE) rodando ponta a ponta pra Graham/Bancos; dividendo médio do Bazin descoberto como exclusivo do plano Pro da bolsai, adiado por decisão do usuário; dois indicadores cripto automatizados sem cadastro (TVL Trend via DefiLlama, Net Issuance via ultrasound.money); active_addresses_trend via Etherscan investigado e descartado (API Pro-only); **CVM (dados abertos) rodando ponta a ponta pro DCF** — 6 dos 13 campos automatizados (EBIT, D&A, Capex, ΔNWC, dívida, caixa; nº de ações vem da bolsai). Retomar por "Pendências pra próxima sessão", item 1)
...
Fase 2 — Coleta de Dados (ações BR + cripto)      [~] Em andamento (cotação via brapi, fundamentos LPA/VPA/ROE via bolsai e fundamentos do DCF via CVM funcionando ponta a ponta; dividendo médio do Bazin bloqueado no plano free da bolsai; TVL Trend (DefiLlama) e Net Issuance (ultrasound.money) automatizados (2 dos 9 indicadores cripto) — ver Log de Sessões; os outros 7 indicadores cripto ainda faltam)
...
- **Marco**: Fase 2.2 completa pro que dá pra automatizar de fontes gratuitas — DCF ganha 6 dos 13 campos automatizados (só `tax_rate` entre os contábeis ficou de fora, não pesquisado; os 5 restantes são premissas de mercado que nunca vêm de balanço). Terceira fatia de coleta desta sessão (depois de bolsai e cripto) — o padrão Python→Rust subprocess→tela se provou de novo, e o hábito de "testar contra dado real antes de fechar" pegou dois problemas reais (VALE3, bug de isolamento) que o planejamento sozinho não teria pego

**Pendências pra próxima sessão** (em ordem):
1. Fase 2, continuação: **`tax_rate` do DCF** (só entrada contábil que sobrou — pesquisar código de conta na DRE, imposto de renda ÷ resultado antes de tributos), **`staking_yield`** (stakingrewards.com — cadastro com free tier ainda não verificado, ver Log de Sessões), ou revisitar o **dividendo médio do Bazin** (investigar brapi com token grátis, ou decidir assinar bolsai Pro). `active_addresses_trend` via Etherscan já foi investigado e descartado (API Pro-only) — não vale reabrir sem uma ideia nova
