...
> Última atualização: 2026-07-12 (Sessão 12, fim — **Fase 5 completa** (5.3 notificação nativa do SO validada de verdade — KDE Plasma 6/Wayland só mostra popup com urgência crítica, `tauri-plugin-notification` trocado por `notify-rust` direto com `.urgency(Urgency::Critical)`) **e Fase 7.1 completa** (storage seguro da chave de API via `keyring` v4/Secret Service, testado de ponta a ponta contra o container real incluindo persistência entre processos separados via KWallet do host, antes de qualquer UI). `cargo test --lib` (42 testes) limpo nas duas fatias.)
...
Fase 7 — Chat de IA Integrado                      [~] Em andamento (7.1 completa — storage seguro da chave via keyring, validado contra o container real; ver Fases Detalhadas)
```
...
1. Fase 7 (chat de IA): seguir pra etapa 7.2 (cliente HTTP do Gemini em Rust via `reqwest` — primeira vez que o Rust do projeto fala com uma API de terceiro direto; reconferir o endpoint/nome do modelo contra a doc real na hora, não usar de memória, ver Fase 7 em "Fases Detalhadas")
