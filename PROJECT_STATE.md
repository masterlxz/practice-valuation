...
> Última atualização: 2026-07-12 (Sessão 15, fim — **Fase 7.4 completa**: painel de chat flutuante (shadcn `Sheet`, botão flutuante, gate de chave de API, lista de mensagens + composer) construído do zero e testado de ponta a ponta pelo usuário no app real — Fase 7 do chat de IA está funcional na tela agora, faltando só multi-provider (7.5+) e, possivelmente, conversas salvas (nova ideia, ver 7.8).)
...
Fase 7 — Chat de IA Integrado                      [~] Em andamento (7.1 a 7.4 completas — storage seguro da chave, cliente HTTP do Gemini, contexto de leitura do banco no systemInstruction e painel de chat na UI, todas validadas ponta a ponta incl. no app real; falta 7.5+ multi-provider; ver Fases Detalhadas)
```
...
1. Fase 7 (chat de IA): 7.5 (abstração multi-provider, `ChatProvider` em Rust — Claude/OpenAI retornam erro tipado "ainda não implementado") é o próximo passo natural da linha principal; **ou** 7.8 (conversas salvas, ideia nova da Sessão 15, ainda não planejada) se o usuário preferir aprofundar a UI antes de expandir provedores — a decidir com o usuário
