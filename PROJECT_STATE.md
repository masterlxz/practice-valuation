...
1. Fase 5.3: assim que der pra rodar `docker compose up` de novo (provavelmente depois de reiniciar a máquina, pra resolver a rede do Docker), rebuildar a imagem (`docker compose build`, o `Dockerfile` mudou) e refazer o smoke test de notificação nativa — criar/forçar uma regra disparando (mesmo truque de cotação simulada) e confirmar que a notificação do SO aparece de verdade. Se não aparecer, o primeiro lugar a olhar é se `DBUS_SESSION_BUS_ADDRESS`/o socket chegaram certos dentro do container (`docker compose exec desktop sh -c 'echo $DBUS_SESSION_BUS_ADDRESS; ls -la /run/user/1000/bus'`) e se o container realmente subiu como uid 1000 (`docker compose exec desktop id`)
