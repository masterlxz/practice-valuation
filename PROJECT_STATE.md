...
1. Fase 2, continuação: escolher a próxima frente — **CVM** (fundamentos pro DCF/RNAV/Bancos, mais complexa: zip anual + parsing de CSV), **`staking_yield`** (stakingrewards.com — cadastro com free tier ainda não verificado, ver Log de Sessões), ou revisitar o **dividendo médio do Bazin** (investigar brapi com token grátis, ou decidir assinar bolsai Pro — ver Log de Sessões, Sessão 5). `active_addresses_trend` via Etherscan já foi investigado e descartado (API Pro-only) — não vale reabrir sem uma ideia nova
